...
> 目标：先做出 Minex 通用驱动外壳（Project 1），验收通过后做 agent 驱动域（Project 2）。
...
- [x] 阶段 1 内核核心（2026-08-12，tag v0.1.0）：registry/events/storage/lifecycle + DriverContext + 19 测试全绿
- [x] 阶段 1 检阅修复（2026-08-12，v0.1.1）：B1激活回滚/B2同优先级先到/M1 destroy容错/M2 reload实现/M3版本比较/M4原子写/M5依赖回滚/M6并发去重；37 测试全绿
- [x] 阶段 2 驱动契约 + demo（2026-08-12，tag v0.2.0）：manifest 解析校验 + loadDriversFromDir（静态贡献自动注册）+ reload + demo 驱动三通道 + 48 测试全绿
- [x] 阶段 2 检阅修复（2026-08-12，v0.2.1）：L1 失败回滚静态贡献/L2 静态-运行时分离生命周期/L3 深层依赖回滚/L4 幂等/L5 逐个容错 + m1-m4/m7/m8；57 测试全绿
- [x] 阶段 3 CLI 宿主（2026-08-12，tag v0.3.0）：@minex/cli（run/config/drivers 三组命令）+ 63 测试全绿 + 端到端通
- [x] 阶段 3 检阅修复（2026-08-12，v0.2.2）：C1 main 激活容错/C2 registry 分层存储（static/runtime）/C3-C5 统一错误捕获；66 测试全绿
- [x] 阶段 4 UI 壳（2026-08-12，tag v0.4.0）：React 布局（顶/左/右/主区/浮窗）+ 槽位 + schema 设置表单 + 命令按钮；浅色蓝主题；vite build + dev 验证通过
...
- [x] 启用/禁用暂存式 + 依赖警告（2026-08-12）：标记待变更 → 重新加载统一应用；禁用被依赖驱动弹确认；对称 applyDriverState
- [x] 主题贡献机制 + 外观驱动（2026-08-12）：ThemeManager 注入驱动 theme CSS；dark 状态提升；packages/appearance-driver（主题色/字体设置 → 浅深双 theme）；sync-drivers 扫描全部驱动包
...
| 1 内核核心 | 能加载驱动、驱动能注册/查询能力 | @minex/kernel：registry / events / storage / lifecycle / kernel | 每原语一提交，tag v0.1.0 |
| 2 驱动契约+demo | 完整生命周期；demo 驱动全流程验证 | manifest / loader / driver-api / reload / demo-driver | tag v0.2.0 |
| 3 CLI 宿主 | 无 UI 也能用内核（Project 2 测功能层的地基） | @minex/cli：加载、执行命令、读写设置 | tag v0.3.0 |
| 4 UI 壳 | 图形界面显示驱动贡献（外观参考 Cherry Studio） | @minex/ui-shell：布局 / 槽位 / schema 设置表单 / 事件刷新 | tag v0.4.0 |
...
- **组件**：
  - 根 `package.json`（npm workspaces：packages/kernel、ui-shell、cli、demo-driver）
  - `tsconfig.json`（统一 TS 配置）
  - `.gitignore`（Node 模板 + Vite 缓存 + 驱动产物）
  - `LICENSE`（MIT）
  - `README.md`（一句话介绍 + 结构图）
...
- **效果**：一个最小脚本能「加载驱动 → 驱动 activate → 注册能力 → query 查到它」。
- **组件**（@minex/kernel，纯 TS，领域无关）：
  - `registry.ts` —— 能力注册表（register/query/get/unregister，priority，{driver} 过滤，onChange）
  - `events.ts` —— 事件总线（emit/on/off）
  - `storage.ts` —— 命名空间存储（get/set/delete/list，JSON 文件实现，可替换）
  - `lifecycle.ts` —— 驱动状态机（discovered/loaded/activated/deactivated）
  - `kernel.ts` —— 组装四件套，暴露两套 API（驱动视图 / 宿主视图）
- **验收**：`npm run test:kernel`（vitest）通过：注册→查询→过滤→priority 覆盖→事件通知。
- **git**：四个原语各一提交；tag `v0.1.0`，push。

## 阶段 2 · 驱动契约 + demo

- **效果**：demo 驱动的静态贡献（manifest）在激活前就可用；动态注册激活后可用；reload 生效；`reloadable:false` 被正确跳过。
- **组件**：
  - `manifest.ts` —— manifest 解析 + 校验（id/version/minKernelVersion/dependencies/settingsSchema/contributes）
  - `loader.ts` —— 动态 `import()` 加载驱动入口
  - `driver-api.ts` —— `activate(ctx)` 的 ctx：register/query/get/on/emit/storage/log
  - `reload.ts` —— 停用→激活；跳过 reloadable:false
  - `demo-driver/` —— 注册 1 个 UI 贡献 + 1 个命令 + 1 个设置项（三条通道各验一次）
- **验收**：CLI 脚本演示全流程；设置值存了能读回来；reload 后贡献刷新。
- **git**：每小块一提交；tag `v0.2.0`，push。

## 阶段 3 · CLI 宿主

- **效果**：命令行能 `minex run demo.sayHello` 执行驱动命令、`minex config get/set` 读写设置。
- **组件**（@minex/cli）：`main.ts`、命令路由、驱动加载、输出格式。
- **验收**：CLI 加载 demo → 执行命令 → 读设置生效。
- **git**：tag `v0.3.0`，push。

## 阶段 4 · UI 壳

- **效果**：图形界面启动后，demo 驱动的贡献显示在槽位；设置表单能改能存；命令能点。外观参考 Cherry Studio。
- **组件**（@minex/ui-shell，React）：
  - 布局（左栏 / 主区 / 顶栏）—— 内核固定，驱动往槽位填内容
