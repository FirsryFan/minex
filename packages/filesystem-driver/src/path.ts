 ...
 * 边界约定（P1-2 文档）：
 * - 空字符串 "" = 根目录（readDir("") 列根）；
 * - `\` 与 `/` 等价（Windows/Web 统一）——本函数把 `\` 归一为 `/`；
 * - `.` 段折叠（"a/./b" → "a/b"）；连续分隔符折叠（"a//b" → "a/b"）；
 * - 绝对路径（`/x`、`C:\x`、`\x`？——`\x` 首段为 "x" 视为相对，见下注）拒绝；
 * - 任何 `..` 段直接拒绝（不做夹紧——宁可报错不可越界）；
 * - 输入为空字符串 → 返回 ""（根）；纯 `..`/`.` 输入被拒绝或归一为 ""。
 * 注：Windows 反斜杠开头的 `\foo` 会被拆成段 ["foo"] 视为相对路径；若需拒绝，调用方用 /^[\\]/ 预检。
 */
